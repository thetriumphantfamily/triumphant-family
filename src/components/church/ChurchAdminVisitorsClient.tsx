...
const FOLLOW_UP_STATUSES = [
  ...
  { value: "visited", label: "🏠 Visited", color: "bg-purple-100 text-purple-700 border-purple-300" },
  { value: "joined", label: "✅ Joined Church", color: "bg-green-100 text-green-700 border-green-300" },
  { value: "lost", label: "❌ Lost Interest", color: "bg-red-100 text-red-700 border-red-300" },
];
...
export default function ChurchAdminVisitorsClient() {
  ...
  const [formData, setFormData] = useState({
    ...
    how_did_you_hear: "", assigned_to: "", visit_date: new Date().toISOString().split("T")[0],
  });
  ...
  const loadVisitors = async () => {
    try {
      ...
      const { data } = await supabase.from("tfam_visitors").select("*").order("created_at", { ascending: false });
      setVisitors(data || []);
      setLoading(false);
  ...
  const resetForm = () => {
    setFormData({ full_name: "", phone: "", email: "", address: "", how_did_you_hear: "", assigned_to: "", visit_date: new Date().toISOString().split("T")[0] });
    setShowForm(false);
  };
  ...
  const updateFollowUp = async (id: string, newStatus: string) => {
    ...
    try {
      ...
      setVisitors((prev) => prev.map((v) => v.id === id ? { ...v, follow_up_status: newStatus, converted_to_member: newStatus === "joined" } : v));
  ...
  if (loading) return <div className="min-h-[400px] flex items-center justify-center"><p className="text-gray-500">Loading visitors...</p></div>;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-brand-purple-900 mb-2">🆕 First-Time Visitors</h1>
          <p className="text-gray-600 text-sm">Track visitors and follow-up progress</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-brand-violet-900 via-brand-purple-800 to-brand-purple-900 border-2 border-brand-gold-400/40 p-5 shadow-xl">
            ...
            <p className="text-4xl font-heading font-bold text-white">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {visitors.length === 0 ? (
        <div className="bg-white rounded-3xl p-10 text-center border-2 border-dashed border-gray-200">
          ...
          <h3 className="font-heading text-xl font-bold text-brand-purple-900 mb-2">No visitors recorded yet</h3>
          <p className="text-gray-500">Click &ldquo;Add Visitor&rdquo; after each service</p>
        </div>
      ) : (
        <div className="space-y-3">
          {visitors.map((visitor) => {
            ...
            return (
              <div key={visitor.id} className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-brand-violet-900 via-brand-purple-800 to-brand-purple-900 border-2 border-brand-gold-400/40 p-5 shadow-xl">
                <div className="absolute top-0 inset-x-0 h-0.5 bg-gradient-to-r from-brand-gold-300 via-brand-gold-400 to-brand-gold-500" />

                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold border ${statusInfo.color}`}>{statusInfo.label}</span>
                    </div>
                ...
                <div className="flex flex-wrap gap-2 pt-3 border-t border-brand-gold-400/30">
                  <select
                    value={visitor.follow_up_status}
                    onChange={(e) => updateFollowUp(visitor.id, e.target.value)}
                    disabled={isBusy}
                    className="px-3 py-1.5 rounded-full bg-brand-purple-950/60 border border-brand-gold-400/40 text-white text-xs font-bold disabled:opacity-50"
                  >
                    {FOLLOW_UP_STATUSES.map((s) => (<option key={s.value} value={s.value}>{s.label}</option>))}
                  </select>
                  <button onClick={() => deleteVisitor(visitor.id)} disabled={isBusy} className="px-3 py-1.5 rounded-full bg-red-500/20 text-red-300 text-xs font-bold disabled:opacity-50">🗑️ Delete</button>
                </div>
              </div>
            );
          })}
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg my-8 pointer-events-auto max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b-2 border-gray-100 p-6 z-10 rounded-t-3xl">
                <div className="flex items-center justify-between">
                  <h2 className="font-heading text-xl font-bold text-brand-purple-900">🆕 Add Visitor</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="date" value={formData.visit_date} onChange={(e) => setFormData({ ...formData, visit_date: e.target.value })} className="w-full p-3 rounded-xl border-2 border-gray-200 focus:border-brand-purple-500 focus:outline-none text-gray-900" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">How Did They Hear About Us?</label>
                  <input type="text" value={formData.how_did_you_hear} onChange={(e) => setFormData({ ...formData, how_did_you_hear: e.target.value })} placeholder="Friend, Social Media, Flyer..." className="w-full p-3 rounded-xl border-2 border-gray-200 focus:border-brand-purple-500 focus:outline-none text-gray-900" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Assigned To (Follow-up)</label>
                  <input type="text" value={formData.assigned_to} onChange={(e) => setFormData({ ...formData, assigned_to: e.target.value })} placeholder="Name of assigned worker" className="w-full p-3 rounded-xl border-2 border-gray-200 focus:border-brand-purple-500 focus:outline-none text-gray-900" />
                </div>
