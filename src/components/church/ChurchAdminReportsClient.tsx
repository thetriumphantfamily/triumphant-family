...
export default function ChurchAdminReportsClient() {
  ...
  const loadStats = async () => {
    try {
      ...
      const [members, approved, pending, visitors, donations, services, devotionals, prayers, answeredPrayers, testimonies, questions, answeredQ, departments, pastoral, announcements] = await Promise.all([
      ...
      const totalDonationAmount = donationData.reduce((sum: number, d: { amount: number }) => sum + Number(d.amount), 0);

      const serviceData = services.data || [];
      const totalAttendance = serviceData.reduce((sum: number, s: { total_attendance: number }) => sum + s.total_attendance, 0);
      const avgAttendance = serviceData.length > 0 ? Math.round(totalAttendance / serviceData.length) : 0;
  ...
  if (loading) return <div className="min-h-[400px] flex items-center justify-center"><p className="text-gray-500">Loading reports...</p></div>;
    ...
    <div className="space-y-6">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-brand-violet-900 via-brand-purple-800 to-brand-purple-900 border-2 border-brand-gold-400/40 p-6 md:p-8 shadow-2xl">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-brand-purple-950/60 border border-brand-gold-400/40 mb-4">
            ...
            <span className="text-white font-black text-sm md:text-base lg:text-lg uppercase tracking-widest">Reports</span>
          </div>
          <h1 className="font-heading text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-3 leading-tight">
            Church Analytics
          </h1>
          <p className="text-brand-purple-100 text-sm md:text-base">Complete overview of your church data</p>
        </div>
      </div>
      ...
      {REPORT_SECTIONS.map((section) => (
        <div key={section.title}>
          <h2 className="font-heading text-lg font-black text-brand-purple-900 mb-3">{section.title}</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {section.items.map((item) => (
              <div key={item.label} className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-brand-violet-900 via-brand-purple-800 to-brand-purple-900 border-2 border-brand-gold-400/40 p-4 shadow-xl">
                <div className="absolute top-0 inset-x-0 h-0.5 bg-gradient-to-r from-brand-gold-300 via-brand-gold-400 to-brand-gold-500" />
                <p className="text-brand-purple-200 text-xs font-semibold uppercase tracking-widest mb-1">{item.label}</p>
