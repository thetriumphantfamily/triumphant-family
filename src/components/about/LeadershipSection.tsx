// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// LEADERSHIP SECTION — White gradient + minimal gold + tight spacing
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
          ...
          {leaders.map((leader) => (
            <div key={leader.id} className="group bg-brand-purple-900/60 rounded-3xl overflow-hidden border-2 border-brand-gold-400/30 hover:border-brand-gold-400/70 transition-all duration-300 hover:-translate-y-1 relative">
              ...
              <div className="relative w-full aspect-[4/5] bg-gradient-to-br from-brand-purple-700 to-brand-violet-900 overflow-hidden">
                {leader.photo_url ? (
                  <img src={leader.photo_url} alt={leader.full_name} className="w-full h-full object-cover object-top group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                ) : (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-28 h-28 rounded-full bg-white flex items-center justify-center text-brand-purple-900 text-3xl font-heading font-bold shadow-md">
                ...
                {leader.title && (
                  <div className="absolute top-5 left-4 bg-white text-brand-purple-900 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-md z-10">
                    {leader.title}
                  </div>
                )}
              </div>
              ...
              <div className="p-5">
                <h3 className="font-heading text-lg font-bold text-white mb-1">{leader.full_name}</h3>
                <p className="text-white font-semibold text-sm mb-3">{leader.role}</p>

                {leader.bio && (
                  <p className="text-brand-purple-100 text-sm leading-relaxed line-clamp-4 mb-3">{leader.bio}</p>
                )}

                {(leader.facebook_url || leader.instagram_url) && (
                  <div className="flex items-center gap-2 pt-3 border-t border-brand-gold-400/30">
                    {leader.facebook_url && (
                      <a href={leader.facebook_url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-brand-purple-900/60 hover:bg-white text-white hover:text-brand-purple-900 flex items-center justify-center transition-all duration-300 border border-brand-gold-400/40" aria-label="Facebook">
                    ...
                    {leader.instagram_url && (
                      <a href={leader.instagram_url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-brand-purple-900/60 hover:bg-white text-white hover:text-brand-purple-900 flex items-center justify-center transition-all duration-300 border border-brand-gold-400/40" aria-label="Instagram">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
