// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// TDA STUDENT SIDEBAR – Portal navigation with notifications badge
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"use client";
...
const NAV_ITEMS = [
  ...
  {
    name: "Notifications",
    href: "/bible-school/portal/notifications",
    hasCount: true,
    icon: (
      <svg fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
...
export default function TDAStudentSidebar() {
    ...
    if (sessionData) {
      try {
        ...
        if (parsed.id) {
          ...
          const interval = setInterval(() => loadUnreadCount(parsed.id), 30000);
  ...
  const loadUnreadCount = async (studentId: string) => {
    try {
  ...
  const handleLogout = () => {
    ...
    setTimeout(() => { router.push("/bible-school/login"); }, 500);
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex-shrink-0 p-4 border-b border-brand-gold-400/20">
          <Link href="/bible-school/portal/dashboard" className="flex items-center gap-3" onClick={() => setMobileOpen(false)}>
            ...
            <div>
              <p className="font-heading font-bold text-white text-sm">TDA Portal</p>
              <p className="text-xs text-brand-purple-200 font-semibold">Student Dashboard</p>
            </div>
          </Link>
        </div>
        ...
        {/* ── Student Info ── */}
        {session && (
          <div className="flex-shrink-0 p-4 border-b border-brand-gold-400/20 bg-brand-purple-950/40">
            <div className="flex items-center gap-3">
              {session.photo_url ? (
                <img src={session.photo_url} alt={session.full_name} className="w-12 h-12 rounded-full object-cover border-2 border-brand-gold-400/40 flex-shrink-0" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white font-black text-sm truncate">{session.full_name}</p>
                <p className="text-brand-purple-200 text-xs font-semibold truncate">{session.student_id}</p>
                <p className="text-brand-purple-200 text-xs font-semibold">Level {session.level}</p>
              </div>
            </div>
          </div>
        )}
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => {
                    setMobileOpen(false);
                    if (item.hasCount && session?.id) loadUnreadCount(session.id);
                  }}
        ...
        <div className="flex-shrink-0 p-3 border-t border-brand-gold-400/20 space-y-2 bg-brand-purple-900/50">
          <Link href="/bible-school" className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-brand-purple-100 hover:bg-white/10 transition-colors text-sm">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
            <span>Bible School Home</span>
          </Link>
          <button
